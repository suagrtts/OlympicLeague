...
public class Orven extends GameCharacter {
    ...
    public void skill2(GameCharacter target){
        if(getSkill2Cooldown() > 0){
            ...
            typewriter("\n" + getName() + " uses VANISH!", 30);
        }
        if(getMana() >= 120){
            this.useMana(120);
            setSkill2Cooldown(3);

            setUntargetable(true); // Become untargetable
            setStatusEffectTurns(1); // Lasts for next turn

            typewriter(getName() + " vanishes from sight, becoming untargetable for the next turn!", 10);
        }else{
            typewriter("Not enough mana!", 30);
        }
    }
    ...
    public void skill3(GameCharacter target){
        if(getSkill3Cooldown() > 0){
            ...
            typewriter("\n" + getName() + " activates HERMES' SPEED!", 30);
        }
        if(getMana() >= 200){
            this.useMana(200);
            setSkill3Cooldown(5);
            ...
            typewriter(getName() + " moves with the speed of Hermes!", 10);
            typewriter("\n>>> First Strike! <<<", 20);
            skill1(target);

            if(target.isAlive()) {
                setSkill1Cooldown(0); // Reset cooldown to allow second attack
                typewriter("\n>>> Second Strike! <<<", 20);
                skill1(target);
            }
        }else{
    ...
    public void takeDamage(int damage) {
        if(getStatusEffectTurns() > 0 && isUntargetable()){
            typewriter(getName() + " evades the attack using VANISH!", 30);
    ...
    public void displayStats() {
        if(getSkill1Cooldown() > 0){
            typewriter("Swift Strike is on cooldown for " + getSkill1Cooldown() + " turns.\n", 10);
        }
        if(getSkill2Cooldown() > 0){
            typewriter("Vanish is on cooldown for " + getSkill2Cooldown() + " turns.\n", 10);
        }
        if(getSkill3Cooldown() > 0){
            typewriter("Hermes' Speed is on cooldown for " + getSkill3Cooldown() + " turns.\n", 10);
        }
        System.out.println();
    ...
    public void takeTurn(GameCharacter target){
        ...
        typewriter("1) Swift Strike - 250 Base Damage - CD: " + getSkill1Cooldown(), 10);
        typewriter("2) Vanish - Untargetable Next Turn - CD: " + getSkill2Cooldown(), 10);
        typewriter("3) Hermes' Speed - Extra Attack This Turn - CD: " + getSkill3Cooldown(), 10);
